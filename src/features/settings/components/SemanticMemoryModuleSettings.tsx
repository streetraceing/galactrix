...
import { SettingsCard } from '../../profile/components/SettingsCard';
import { ModuleNumberField, ModuleProviderSelect } from './ModuleFields';
...
export function SemanticMemoryModuleSettings({
  ...
  return (
    ...
    >
      <SettingSwitchRow
        label={t('ai.semantic.enabled')}
        description={t('ai.semantic.enabledDescription')}
        value={value.enabled}
        ...
        <>
          ...
          {value.indexArchivedMessages ? (
            <ModuleNumberField
              ...
              step={20}
              onChange={(next) =>
                patch('archivedMessageLimit', Math.round(next))
              }
            />
          ) : null}
        </>
