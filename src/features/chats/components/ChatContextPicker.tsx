import { Button, Chip, Label, ListBox, Select, Surface } from '@heroui/react';
...
export function ChatContextPicker({
  ...
  const styles = ofKind(galaxyItems, 'style');
  ...
  const selectedCount =
    ...
    Number(Boolean(value.styleItemId)) +
  ...
  return (
    <Surface className="min-w-0 rounded-2xl border border-separator p-3 sm:p-5 bg-surface-secondary/50">
      <div
        className={`flex min-w-0 items-start justify-between gap-3 ${
          !isCompactLayout || expanded ? 'mb-3' : ''
        }`}
      >
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          {isCompactLayout ? (
            ...
            >
              {expanded
                ? t('chatContextPicker.collapse')
                : t('chatContextPicker.expand')}
            </Button>
          ) : null}
        </div>
      </div>
      ...
      {!isCompactLayout || expanded ? (
        <div className="flex min-w-0 flex-col gap-3 sm:gap-4">
          ...
          />
          <GalaxySelectField
            label={t('chatContextPicker.responseStyle')}
            placeholder={t('chatContextPicker.useCharacterStyle')}
            items={styles}
            value={value.styleItemId}
            onChange={(styleItemId) => onChange({ ...value, styleItemId })}
          />
          <GalaxySelectField
      ...
      {galaxyItems.length === 0 && (!isCompactLayout || expanded) ? (
        <div className="mt-3 flex items-start gap-2 border-t border-separator pt-3 sm:mt-4 sm:pt-4 text-xs leading-5 text-muted">
