...
3. `db.rs` coordinates persistence, while `db/galaxy.rs`, `db/settings.rs` (settings, usage history, budgets, snippets, budget-status aggregation over UTC-aligned periods), `db/ai_memory.rs`, `db/health.rs` (integrity reports and safe repairs) and `db/revisions.rs` (bounded edit history with undoable restores, tuning-hint queries over rated variants) own focused storage domains.
4. `provider_client.rs` owns provider operations: `complete_streaming` consumes SSE (OpenAI-compatible) and ndjson (Ollama) payloads incrementally with a `StreamChunkParser`, falls back to buffered parsing when a provider ignores `stream`, and honours cancellation by returning partial text; endpoint policy and retry/rate-limit transport (including the raw-response variant used by streaming) live in dedicated submodules. `stream_with_fallback` in `lib.rs` walks a per-chat provider chain (primary plus ordered fallbacks from `ChatGenerationSettings.fallbackProviderIds`) while no text has been streamed, and the response report records the provider that actually served the reply.
...
- Generation streams through a scoped `tauri::ipc::Channel<StreamDelta>`; deltas are batched on the backend (size/time flush) and appended into the optimistic message on the frontend. Cancellation keeps partial text; a mid-stream failure never falls back to another provider, only pre-stream failures do.
- Budget rules (`settings.budgets`) are checked before every generation starts via `get_budget_status`; exceeding one warns with a toast but never blocks the request. Period boundaries are UTC-aligned with the usage-history day buckets.
- Composer snippets and app budgets are device settings: they ride the full backup through `settings`, but have no cross-device sync of their own.
