...
const statePath = new URL(
  '../../src/app/chatNavigationState.ts',
  import.meta.url,
);

test('chat navigation persistence drops legacy scroll data', async () => {
  ...
  assert.match(source, /removeStorageItem\(LEGACY_CHAT_VIEW_STORAGE_KEY\)/);
